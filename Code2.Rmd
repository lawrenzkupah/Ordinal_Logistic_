...
stargazer(QDEP.2,QINTR.2,AEFT.2,AHOPS.2,ARES.2,ASAD.2,AWORS.2,apply.coef = exp,style = "qje", keep.stat = "res.dev",type = "text",add.lines = c(c('1','2'),c('1','3')))

stargazer(QDEP.2,QINTR.2,AEFT.2,AHOPS.2,ARES.2,ASAD.2,AWORS.2,style = "qje",apply.coef = exp, type = "text")
...
stargazer(QDEP.3 ,QINTR.3,AEFT.3 ,AHOPS.3,ARES.3,ASAD.3,AWORS.3, keep = keep.1,apply.coef = exp,style = "qje")
```

```{r Models 1 -- non quadratic}
QDEP.3Ln <- polr(PHQDEP ~ INCTOT +  FTOTINCMEPS + YEAR + REGIONMEPS +SEX + MARSTAT + HEALTH + 
                  EDUC + SMOKENOW + RACEA + CANCEREV + INTERVLANG + FAMSIZE +ANERVOUS,
                  data = panel, Hess = TRUE)
summary(QDEP.3)
AEFT.3Ln <-polr(AEFFORT ~ INCTOT + FTOTINCMEPS + YEAR + REGIONMEPS +SEX + MARSTAT + HEALTH + 
                           EDUC + SMOKENOW + RACEA + CANCEREV + INTERVLANG + FAMSIZE + ANERVOUS,
                          data = panel, Hess = TRUE)
AHOPS.3Ln <-polr(AHOPELESS ~ INCTOT + FTOTINCMEPS + YEAR + REGIONMEPS +SEX + MARSTAT + HEALTH + 
                         EDUC + SMOKENOW + RACEA + CANCEREV + INTERVLANG + FAMSIZE + ANERVOUS,
                       data = panel, Hess = TRUE)
ARES.3Ln <-polr(ARESTLESS ~ INCTOT + FTOTINCMEPS + YEAR + REGIONMEPS +SEX + MARSTAT + HEALTH + 
                         EDUC + SMOKENOW + RACEA + CANCEREV + INTERVLANG + FAMSIZE + ANERVOUS,
                       data = panel, Hess = TRUE)
ASAD.3Ln <-polr(ASAD ~ INCTOT + FTOTINCMEPS + YEAR + REGIONMEPS +SEX + MARSTAT + HEALTH + 
                         EDUC + SMOKENOW + RACEA + CANCEREV + INTERVLANG + FAMSIZE + ANERVOUS,
                       data = panel, Hess = TRUE)
AWORS.3Ln <-polr(AWORTHLESS ~ INCTOT + FTOTINCMEPS + YEAR + REGIONMEPS +SEX + MARSTAT + HEALTH + 
                         EDUC + SMOKENOW + RACEA + CANCEREV + INTERVLANG + FAMSIZE + ANERVOUS,
                       data = panel, Hess = TRUE)
QINTR.3Ln <-polr(PHQINTR ~ INCTOT + FTOTINCMEPS + YEAR + REGIONMEPS +SEX + MARSTAT + HEALTH + 
                         EDUC + SMOKENOW + RACEA + CANCEREV + INTERVLANG + FAMSIZE + ANERVOUS,
                       data = panel, Hess = TRUE)

aicVals <- AIC(QDEP.3Ln)
aicVals <- c(AIC(AEFT.3Ln),aicVals)
aicVals <- c(AIC(AHOPS.3Ln),aicVals)
aicVals <- c(AIC(ARES.3Ln),aicVals)
aicVals <- c(AIC(ASAD.3Ln),aicVals)
aicVals <- c(AIC(AWORS.3Ln),aicVals)
aicVals <- c(AIC(QINTR.3Ln),aicVals)

stargazer(QDEP.3Ln ,QINTR.3Ln,AEFT.3Ln,AHOPS.3Ln,ARES.3Ln,ASAD.3Ln,AWORS.3Ln, apply.coef = exp,style = "qje")

keep.1 <- c("INCTOT","FTOTINCMEPS","SEX2","MARSTAT30","HEALTH5","EDUC400","SMOKENOW2","RACE200","CANCEREV2","INVERVLANG2","ANERVOUS1","INCTOT2","FTOTINCMEPS2")
stargazer(QDEP.3Ln ,QINTR.3Ln,AEFT.3Ln ,AHOPS.3Ln,ARES.3Ln,ASAD.3Ln,AWORS.3Ln, keep = keep.1,apply.coef = exp,style = "qje")
...
stargazer(QDEP.4,QINTR.4,AEFT.4,AHOPS.4,ARES.4,ASAD.4,AWORS.4, apply.coef = exp,style = "qje")
...
stargazer(QDEP.4,QINTR.4,AEFT.4,AHOPS.4,ARES.4,ASAD.4,AWORS.4, keep = keep.2,apply.coef = exp,style = "qje",ord.intercepts = "TRUE")
```
...
stargazer(QDEP.5,QINTR.5,AEFT.5,AHOPS.5,ARES.5,ASAD.5,AWORS.5, apply.coef = exp,style = "qje")
...
stargazer(QDEP.5,QINTR.5,AEFT.5,AHOPS.5,ARES.5,ASAD.5,AWORS.5, keep = keep.3,apply.coef = exp,style = "qje",ord.intercepts = "TRUE")
