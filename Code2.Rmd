...
```{r model .5}
panel$INCTOT <- panel$INCTOT/10 #(data is alredy divided by 100, adding another 10, because otherwise summary doesnt want to compute)
...
#Between here is just me testing things 
se.coef <- function(glm.output){sqrt(diag(vcov(glm.output)))}

OR <- exp(coef(QDEP.2))
Std.Error.OR_QDEP.2 <-  OR * se.coef(QDEP.2)
Std.Error.OR_QDEP.2
stargazer((QDEP.2),apply.coef = exp,style = "qje",type = "text",p.auto = F,t.auto = F,se = list(Std.Error.OR_QDEP.2)) #w/ SE

#seeing what a summary of QDEP.2 looks like 
st <- summary(QDEP.2)
p.values <- list(summary(QDEP.2)$coefficients[,3])
p.values

stargazer((QDEP.2),apply.coef = exp,style = "qje",type = "text",p.auto = F,t.auto = F, #try CI's
 ci = T)
...
aicVals <- AIC(QDEP.2)
aicVals <- c(AIC(AEFT.2),aicVals)
aicVals <- c(AIC(AHOPS.2),aicVals)
aicVals <- c(AIC(ARES.2),aicVals)
aicVals <- c(AIC(ASAD.2),aicVals)
aicVals <- c(AIC(AWORS.2),aicVals)
aicVals <- c(AIC(QINTR.2),aicVals)

print(aicVals)

print('QDEP.2  AEFT.2  AHOPS.2  ARES.2  ASAD.2  AWORS.2  QINTR.2')

stargazer(QDEP.2,QINTR.2,AEFT.2,AHOPS.2,ARES.2,ASAD.2,AWORS.2,apply.coef = exp,style = "qje",ci = T,p.auto = F)

stargazer(QDEP.2,QINTR.2,AEFT.2,AHOPS.2,ARES.2,ASAD.2,AWORS.2,apply.coef = exp,style = "qje", type = 'text', ci = T, p.auto = F)
...
aicVals <- AIC(QDEP.3)
aicVals <- c(AIC(AEFT.3),aicVals)
aicVals <- c(AIC(AHOPS.3),aicVals)
aicVals <- c(AIC(ARES.3),aicVals)
aicVals <- c(AIC(ASAD.3),aicVals)
aicVals <- c(AIC(AWORS.3),aicVals)
aicVals <- c(AIC(QINTR.3),aicVals)

print('QDEP.3  AEFT.3  AHOPS.3  ARES.3  ASAD.3  AWORS.3  QINTR.3')

print(aicVals)

stargazer(QDEP.3 ,QINTR.3,AEFT.3 ,AHOPS.3,ARES.3,ASAD.3,AWORS.3, apply.coef = exp,style = "qje",ci=T, p.auto= F)
...
stargazer(QDEP.3 ,QINTR.3,AEFT.3 ,AHOPS.3,ARES.3,ASAD.3,AWORS.3, keep = keep.1,apply.coef = exp,style = "qje",ci = T, p.auto = F)

stargazer(QDEP.3 ,QINTR.3,AEFT.3 ,AHOPS.3,ARES.3,ASAD.3,AWORS.3, keep = keep.1,apply.coef = exp,style = "qje",ci = T, p.auto = F,type= "text")
```

```{r Models 1 -- non quadratic}
...
stargazer(QDEP.3,QINTR.3,AEFT.3,AHOPS.3,ARES.3,ASAD.3,AWORS.3, apply.coef = exp, style = "qje")
...
panel.2 <- panel[, names(panel) %in% c("INCTOT","FTOTINCMEPS","YEAR","REGIONMEPS","SEX","MARSTAT","HEALTH",
                                       "EDUC","SMOKENOW","RACEA","CANCEREV","INTERVLANG","FAMSIZE","ANERVOUS",
                                       "fracfamInc","capGains","fraccapgains","PHQDEP","AHOPELESS","ASAD","AWORTHLESS","AEFFORT","ARESTLESS","PHQINTR","FTOTINCMEPS2","INCTOT2")]
panel.2 <- na.omit(panel.2)
...
aicVals <- AIC(QDEP.4)
aicVals <- c(AIC(AEFT.4),aicVals)
aicVals <- c(AIC(AHOPS.4),aicVals)
aicVals <- c(AIC(ARES.4),aicVals)
aicVals <- c(AIC(ASAD.4),aicVals)
aicVals <- c(AIC(AWORS.4),aicVals)
aicVals <- c(AIC(QINTR.4),aicVals)

print('QDEP.4  AEFT.4  AHOPS.4  ARES.4  ASAD.4  AWORS.4  QINTR.4')

print(aicVals)

stargazer(QDEP.4,QINTR.4,AEFT.4,AHOPS.4,ARES.4,ASAD.4,AWORS.4, apply.coef = exp,style = "qje", ci=T, p.auto= F)
...
stargazer(QDEP.4,QINTR.4,AEFT.4,AHOPS.4,ARES.4,ASAD.4,AWORS.4, keep = keep.2,apply.coef = exp,style = "qje", ci = T,p.auto = F)

stargazer(QDEP.4,QINTR.4,AEFT.4,AHOPS.4,ARES.4,ASAD.4,AWORS.4, keep = keep.2,apply.coef = exp,style = "qje", ci = T,p.auto = F, type = 'text')
```
...
aicVals <- AIC(QDEP.5)
aicVals <- c(AIC(AEFT.5),aicVals)
aicVals <- c(AIC(AHOPS.5),aicVals)
aicVals <- c(AIC(ARES.5),aicVals)
aicVals <- c(AIC(ASAD.5),aicVals)
aicVals <- c(AIC(AWORS.5),aicVals)
aicVals <- c(AIC(QINTR.5),aicVals)

print('QDEP.5  AEFT.5  AHOPS.5  ARES.5  ASAD.5  AWORS.5  QINTR.5')

print(aicVals)

stargazer(QDEP.5,QINTR.5,AEFT.5,AHOPS.5,ARES.5,ASAD.5,AWORS.5, apply.coef = exp,style = "qje",ci=T, p.auto= F)
...
stargazer(QDEP.5,QINTR.5,AEFT.5,AHOPS.5,ARES.5,ASAD.5,AWORS.5, keep = keep.3,apply.coef = exp,style = "qje", ci = T,p.auto = F)

stargazer(QDEP.5,QINTR.5,AEFT.5,AHOPS.5,ARES.5,ASAD.5,AWORS.5, keep = keep.3,apply.coef = exp,style = "qje", ci = T,p.auto = F, type = 'text')
